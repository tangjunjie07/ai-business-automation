...
def classify_transactions_with_claude(
    ...
    if predictor is None:
        try:
            ...
            logger.warning("ClaudePredictor is not available: %s", e)
            predictor = None
...
def _get_default_predictor(predictor: Optional[ClaudePredictor]) -> Optional[ClaudePredictor]:
    if predictor is not None:
        return predictor
    try:
        return ClaudePredictor()
    except Exception as e:
        logger.info("ClaudePredictor not available; skipping classification (%s)", e)
        return None
...
def classify_transactions(
    ...
    *,
    ...
    利用不可なら no-op で返します。
    """
    resolved = _get_default_predictor(predictor)
    return classify_transactions_with_claude(txs, predictor=resolved)
...
async def persist_transactions_to_db(
    *,
    ...
    from app.account_classifier.db_service import (
        ...
        convert_transaction_to_journal_data,
    )
    ...
    for idx, tx in enumerate(classified_txs, 1):
        try:
            ...
            prediction_result = {
                ...
                # マスタ照合の confidence があれば優先
                "confidence": tx.get("account_confidence") if tx.get("account_confidence") is not None else tx.get("confidence"),
                ...
                "raw_response": tx.get("claude_raw_response"),
                "model": tx.get("claude_model"),
            ...
            journal_data = convert_transaction_to_journal_data(tx)
            await mf_service.save_journal_entry(
                tenant_id=tenant_id,
                journal_data=journal_data,
                claude_prediction_id=prediction_id,
            )
            saved += 1
        except Exception as e:
...
def run_pipeline(
    *,
    ...
    resolved_predictor: Optional[ClaudePredictor] = None
    if predictor == "claude":
        resolved_predictor = _get_default_predictor(None)

    classify_transactions_with_claude(txs, predictor=resolved_predictor)
